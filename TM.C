...
typedef enum {
   ...
   opLDF,     /* RA     reg(r) = d ; reg(s) is ignored */
...
char * stepResultTab[]
        = {"OK","Halted","Instruction Memory Fault",
           "Data Memory Fault","Division by 0","Remainder Operation only works with two integer arguments"
          };
...
{ int sign;
  ...
  { sign = 1;
    ...
    if (ch == '.') {
        ...
        clonedNum.type = 'i';
        clonedNum.num.valuei = num + (term * sign);
...
{ OPCODE op;
  ...
  for (regNo = 0; regNo < NO_REGS; regNo++) {
      reg[regNo].num.valuei = 0;
      reg[regNo].type = 'i';

  }
  dMem[0].num.valuei = DADDR_SIZE - 1 ;
  dMem[0].type = 'i';

  for (loc = 1; loc < DADDR_SIZE; loc++) {
      dMem[loc].num.valuei = 0;
      dMem[loc].type = 'i';
...
{ INSTRUCTION currentinstruction  ;
  ...
  pc = reg[PC_REG].num.valuei ;
  if ( (pc < 0) || (pc > IADDR_SIZE)  )
      return srIMEM_ERR ;
  reg[PC_REG].num.valuei = pc + 1 ;
  reg[PC_REG].type = 'i';
  ...
  { /* RR instructions */
    ...
    /***********************************/
      ...
      { //printf("Enter value for IN instruction: ") ;
        ...
        if (!ok) {printf("Illegal value\n");
            ...
            //reg[r] = num;
            if (clonedNum.type == 'f') {
                reg[r].num.valuef = clonedNum.num.valuef;
                reg[r].type = 'f';
            }
            else {

                reg[r].num.valuei = clonedNum.num.valuei;
                reg[r].type = 'i';
            }
        }
      }
    ...
    case opDIV :
    /***********************************/
      //if ( reg[t] != 0 ) reg[r] = (int)reg[s] / (int)reg[t];
      //else return srZERODIVIDE ;
      
              if (reg[s].type == 'i' && reg[t].type == 'i') {
                  if (reg[t].num.valuei != 0) {
                      reg[r].num.valuei = reg[s].num.valuei / reg[t].num.valuei;
                      reg[r].type = 'i';
                  }
                  else {
    ...
    case opRES:
        /***********************************/
            ...
            else return srNOINTEGERSRESOP;
        
       break;
    /*************** RM instructions ********************/
    case opLD :
        if (dMem[m].type == 'i') {
            reg[r].num.valuei = dMem[m].num.valuei;
            reg[r].type = 'i';
        }
        else {
            reg[r].num.valuef = dMem[m].num.valuef;
            reg[r].type = 'f';
        }
        break;
    case opST :
        //dMem[m] = reg[r] ;  
        if (reg[r].type == 'i') {
            dMem[m].num.valuei = reg[r].num.valuei;
            dMem[m].type = 'i';
        }
        else {
            dMem[m].num.valuef = reg[r].num.valuef;
            dMem[m].type = 'f';
        }
        break;
    ...
    case opLDA :    
        ...
        break;
    case opLDC :
        reg[r].num.valuei = (int)currentinstruction.iarg2;
        reg[r].type = 'i';
        break;
    case opLDF :

        reg[r].num.valuef = currentinstruction.iarg2 ;
        reg[r].type = 'f';
        break;
    case opJLT: regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox < 0) reg[PC_REG].num.valuei = m;reg[PC_REG].type = 'i'; break;
    case opJLE : regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox <=  0 ) reg[PC_REG].num.valuei = m ;reg[PC_REG].type = 'i'; break;
    case opJGT : regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox >  0 ) reg[PC_REG].num.valuei = m ;reg[PC_REG].type = 'i'; break;
    case opJGE : regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox >=  0 ) reg[PC_REG].num.valuei = m ;reg[PC_REG].type = 'i'; break;
    case opJEQ : regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox == 0 ) reg[PC_REG].num.valuei = m ;reg[PC_REG].type = 'i'; break;
    case opJNE : regBox = 0; if (reg[r].type == 'i') regBox = reg[r].num.valuei;else regBox = reg[r].num.valuef;
        if (regBox != 0 ) reg[PC_REG].num.valuei = m ;reg[PC_REG].type = 'i'; break;
